#####################################################################################################
# This code file contains all functions used to implement the MCCI method.                          #
...
library(doSNOW)
library(foreach)
simulate_datapath_f <- '/random_samples_generation/'
...
dodiffcore <- function(){
  ...
  dataname <- cur1[1]
  curoutfile <- paste0(fileout,'/',curname)
  cur_result <- readModels(curoutfile,what="all")
  curdiff <- getdiff(cur_result)
  c(dataname,curdiff);
}

dodiff <- function(pinpath,numpara,curpath) {
...
main_flow_combine_sample_est <- function(curpath, empirical_datapath, model_title, model_variable, model_model, model_output, model_savedata) {
  setwd(curpath)
  simulate_datapath <- paste0(curpath,simulate_datapath_f)
  ...
  combined_estimates <- read.csv(paste0(simulate_datapath,"combined_sample_estimates.dat"), header = FALSE)
  copy_estimates <- combined_estimates
  population_values<-rbind(combined_estimates,copy_estimates)
  write.table(population_values, paste0(simulate_datapath,"combined_sample_estimates.dat"), row.names = FALSE, col.names = FALSE,quote = FALSE)
}

## function for performing random samples generation
main_flow_random_samples <- function(curpath, model_title, model_montecarlo, model_population) {
  setwd(curpath)
  simulate_datapath <- paste0(curpath,simulate_datapath_f)
...
#######################################################################################
meta_flow_core <- function(curpath, empirical_datapath, tagout_datapath, process_tag, model_title, model_variable, model_model, model_output, item_num, factor_num) {
  ...
  simulate_outpath <- paste0(tagout_datapath,'/simulateddata_output')
  empirical_outpath <- paste0(tagout_datapath,'/empiricaldata_output')
  simulate_datapath <- paste0(curpath,simulate_datapath_f)
...
############## Step 2:use the Monte Carlo method to approximate the null distribution of the differences in the parameters ##############
  ...
  setwd(curpath)
  dodiff(simulate_outpath,para_num,curpath)
...
############## Step 4: evaluate the invariance of each item ##############    
  ...
  write.csv(result_by_tag, file=paste0(curpath,"/",process_tag,"_testresults.csv"))
  stopCluster(clusterobj)
  ##end function
}

#################### testing for metric invariance (factor loading) ################
main_flow_loading_1 <- function(curpath, empirical_datapath, model_title, model_variable, model_model, model_output, item_num, factor_num) {
  setwd(curpath)
  process_tag <- 'loading'
  loading_outpath <- paste0(curpath,'/loading_outputfile/')
  createDirForOutput(loading_outpath)
  meta_flow_core(curpath, empirical_datapath, loading_outpath, process_tag, model_title, model_variable, model_model, model_output, item_num, factor_num)
}

#################### testing for scalar invariance (intercept) #####################
main_flow_intercept_2 <- function(curpath, empirical_datapath, model_title, model_variable, model_model, model_output, item_num, factor_num) {
  setwd(curpath)
  process_tag <- 'intercept'
  loading_outpath <- paste0(curpath,'/intercept_outputfile/')
  createDirForOutput(loading_outpath)
  meta_flow_core(curpath, empirical_datapath, loading_outpath, process_tag, model_title, model_variable, model_model, model_output, item_num, factor_num)
}

#################### testing for strict invariance (residual variance) #############
main_flow_residualvariance_3 <- function(curpath, empirical_datapath, model_title, model_variable, model_model, model_output, item_num, factor_num) {
  setwd(curpath)
  process_tag <- 'residualvariance'
  loading_outpath <- paste0(curpath,'/residualvariance_outputfile/')
  createDirForOutput(loading_outpath)
  meta_flow_core(curpath, empirical_datapath, loading_outpath, process_tag, model_title, model_variable, model_model, model_output, item_num, factor_num)
}